...
"""
    ...
    parts.append(
        """/* ---- Shell brand tokens (hero gradient; overridable per client) ---- */
:root {
...
.hero-card { background: var(--primary-container-background); color: var(--foreground); border: 1px solid var(--border); border-radius: var(--radius-lg); padding: 36px 32px; }
/* dark hero with background imagery: dark gradient multiplied over --hero-img (set inline per brief; falls back to a dark fill) */
.hero-card.has-img { background-image: var(--hero-overlay), var(--hero-img, linear-gradient(#0E1421, #0E1421)); background-size: cover; background-position: center; color: #fff; border-color: transparent; display: flex; flex-direction: column; justify-content: center; }
.hero-card.has-img h1 { color: #fff; }
.hero-card.has-img .lede { color: #fff; opacity: .92; }
.hero-card.has-img .eyebrow { background: rgba(255,255,255,.12); border-color: rgba(255,255,255,.28); color: #fff; align-self: flex-start; }
.hero .eyebrow { display: inline-block; background: var(--card); border: 1px solid var(--border); border-radius: var(--radius-full); padding: 4px 14px; font-size: var(--font-size-12); font-weight: var(--font-semibold); letter-spacing: 0.02em; color: var(--muted-foreground); margin-bottom: 20px; }
.hero h1 { margin-bottom: 14px; }
.hero .lede { font-size: var(--font-size-16); line-height: var(--font-line-height-16); color: var(--foreground); opacity: .82; max-width: 680px; margin: 0; }
